...
- `prototipo/sedef-test/`: grande atualização do protótipo com novos componentes e páginas.
  ...
  - `src/data/monitoramento.ts`: dados de exemplo de monitoramento — simulação de histórico de indicadores e status de tarefas por compromisso, cobrindo 10 anos de dados para satisfazer RNF07.
  - `src/pages/Monitoramento.tsx`: tela de monitoramento completa com exibição de status de tarefas, valores de indicadores, responsáveis e última atualização; posterior adição do botão de exportar dados com seleção de formato (Excel, CSV, PDF).
  - Novos componentes: `GraficoIndicador.tsx` (gráfico de progresso por indicador), `HistoricoTarefa.tsx` (histórico de atualizações de tarefa com data e conteúdo anterior/novo), `IndicadorChart.tsx` (gráfico de indicadores filtrável por ano), `ResponsaveisSection.tsx` (Responsável Principal e Colaboradores por ação), `StatusBadge.tsx` (badge visual de status de tarefa com diferenciação por tipo) e `UltimaAtualizacao.tsx` (carimbo de data e hora da última atualização por item).
  - `src/App.tsx`: roteamento atualizado para incluir a tela de monitoramento.
  - `src/components/Header.tsx`: navegação atualizada com link para a tela de monitoramento.
  - `src/pages/Home.tsx`: ajustes de layout para integrar o acesso à tela de monitoramento.
  - Build do protótipo (`dist/`) atualizado.
- `prototipo/docs/tela-*.md` e `prototipo/docs/validacao-prototipo.md`: 11 arquivos vazios removidos (limpeza do repositório).
...
- `docs/conversa-cliente.md`: atualização com novas informações coletadas do cliente sobre o fluxo de aprovação de tarefas e sobre a área infantojuvenil; mapeamento de cada pergunta do roteiro de conversa para o RF/RNF correspondente, tornando explícita a rastreabilidade entre a entrevista com o cliente e os requisitos do sistema.
- `docs/requisitos.md`: correções de pontuação e numeração, remoção de comentários internos, alterações de conteúdo em RF04, RF20, RF22, RF23, RF24, adição de RF28, renumeração a partir de RF15 e adição de nota de consistência esclarecendo a distinção entre RF07/RF08/RF13 e entre RF09/RF24; detalhamento adicional de RF20, RF24, RF26, RF27 e RF28; correção de referências cruzadas entre requisitos.
- `docs/casos-uso.md`: renumeração de todos os casos de uso (UC01–UC19) para refletir as mudanças de numeração dos RFs; adição de UC20, cobrindo o novo RF28 (seção "Procure ajuda" para adolescentes).
- `docs/criterios-aceite.md`: renumeração dos RFs nas referências e refinamento dos critérios de aceite para refletir as mudanças de requisitos. Ajustes notáveis:
...
- `docs/historias-usuario.md`: expansão e reestruturação completa das histórias por perfil. A versão anterior tinha 8 histórias genéricas; a nova versão tem ~20 histórias organizadas em seções por perfil (Público Geral, Conselho/Secretarias, SEDEF/Administrador, Crianças e Adolescentes, Usuário PCD), cada uma vinculada explicitamente ao RF/RNF de origem.
- `docs/regras-negocio.md`: reorganização completa das regras de negócio por requisito de origem, tornando rastreabilidade bidirecional (RF → RN e RN → RF). Atualização da matriz de permissões para referenciar RF24 (em vez de RF23 antigo) e adição de regras para os novos RFs.
...
RF04: sobreposição com RF05 tornava a divisão busca/filtro ambígua para implementação e verificação. RF19→RF20: "no mínimo três" abria margem para perfis não planejados, dificultando modelagem de banco de dados e testes de perfil. RF22 antigo (JWT): requisitos funcionais devem descrever o *quê* (comportamento), não o *como* (tecnologia de implementação). RF23→RF24 (matriz): a versão anterior era genérica demais para servir de critério de teste; a matriz detalha cada permissão verificável de forma independente. RF26→RF27 + RF28: o antigo RF27 misturava dois escopos distintos (conteúdo educativo e canal de socorro); a separação permite verificar cada um de forma autônoma. Reorganização de historias-usuario, criterios-aceite e regras-negocio: necessária para manter todos os artefatos coerentes após as mudanças de numeração dos RFs.
 
### Impactos das mudanças:
- Protótipo passa a cobrir: RF01 (23 problemas públicos na página principal), RF02 (hierarquia completa expansível), RF03 (monitoramento de status e indicadores), RF06 (exportação), RF07 (última atualização por item), RF08 (histórico de tarefa), RF16 (gráficos de indicadores), RF17 (filtro por ano), RF18 (diferenciação visual por tipo), RF19 (responsáveis por ação), RNF01 (acessibilidade via ShadcnUI), RNF02 (responsividade) e RNF06 (consistência visual com CEDCA-PR).
- Dataset simulado em `monitoramento.ts` demonstra suporte ao histórico de 10 anos exigido por RNF07.
- Conversa com cliente atualizada subsidia as revisões de requisitos realizadas na própria semana.
- Renumeração a partir de RF15 impactou todos os casos de uso (UC01–UC20 renumerados).
